...
eniOil.prototype.render = function () {
    this.app.contentTag.innerHTML =
        '<div class="container"> \
            <div id="layer-1" class="layer" ><img src="' + this.app.path + 'images/eo-img-300x250-01-01.png"/></div> \
            <div id="layer-2" class="layer"> \
                <img src="' + this.app.path + 'images/eo-img-300x250-02-01.png"/> \
                <form id="form" class="form"> \
                    <input type="text" id="name" placeholder="Nama:" required> \
                    <input type="tel" id="hp" placeholder="No HP:" required> \
                    <div class="last"> \
                        <button id="submit" class="btn"><img src="' + this.app.path + 'images/button-img.png"/></button> \
                    </div> \
                </form> \
            </div> \
            <div id="layer-3" class="layer"> \
                <img src="' + this.app.path + 'images/eo-img-300x250-03-01.png"/> \
            </div> \
        </div>';
}
...
eniOil.prototype.style = function () {
    ...
    css += '#layer-1 { display: block; } #layer-1 img { width: 300px; height: 250px; position: relative; cursor: pointer; } ';
    css += '#layer-2 { display: none; } ';
    css += '#layer-3 { display: none; } ';
    css += '.layer { width: 300px; height: 250px; position: absolute; top: 0; left: 0;} ';
    css += '.form { position: absolute; bottom: 30px; left: 0; text-align: center; } .form input { width: 208px; height: 30px; padding: 0 10px; font-size: 16px; } .form .last { margin-top: 10px; } ';
    css += '.btn { background: none; border: 0; }';
    css += '#submit img { position: absolute; left: 130px; bottom: -19px;}';
...
eniOil.prototype.events = function () {
    ...
    this.form.addEventListener('submit', function (e) {
        ...
        if (/\S/.test(name) && /\S/.test(hp)) {
            ...
            var url = 'https://www.mobileads.com/api/save_lf?contactEmail=dickale@imx.co.id,karima@imx.co.id,adhie@mobileads.com&gotDatas=1&element=[{%22fieldname%22:%22text_1%22,%22value%22:%22' + name + '%22},{%22fieldname%22:%22text_2%22,%22value%22:%22' + hp + '%22}]&user-id=2901&studio-id=214&tab-id=1&trackid=2076&referredURL=Sample%20Ad%20Unit&callback=leadGenCallback'
            /* tracker */
